...
### Details
The compiler uses a lexical analyzer built using Thompson constrution (adding support Gluskhov's construction is in the plans), and a syntax analyzer built using LR(1) parsing. Syntax tree produced by the syntax analyzer is converted to AST (Abstract Syntax Tree) and then intermediate code is generated, IR is inspired by [LLVM](https://github.com/llvm/llvm-project) IR. IR code is translated to X64 NASM. The standard library is partly implemented and can be seen in [src/std](src/std) folder.
### How to use
...
```
Then `output.nasm` file is created, which can compile:
```shell
...
```
### Test
The project has tests for lexical and syntax analyzer, to run the tests:
```shell
